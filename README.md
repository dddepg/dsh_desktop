...
- **内置 10 款社区皮肤** — 设置 → 皮肤 浏览切换（blue-fantasy / dragon-heir / maid-atelier / miku / minecraft / qq98 / ths / trading / whale-song / xp）；M3 皮肤首次启动默认启用
- **侧边栏 GitGraph** — VSCode 风格分支地图概览，自动识别外层 + 内层双仓库并支持一键切换
- **推理强度滑块** — 六档 `Off/Low/Medium/High/Extra/Max`，Claude 风格动画 + HanaAyane chibi runner 拇指形象
...
| `dsh-gitgraph` | 侧边栏 GitGraph 分支地图（内外双仓库切换） | 内置 |
| `dsh-skin-switch` | 设置页「皮肤」tab：列出并切换内置皮肤 | 内置 |
| `dsh-client-ui-effort-slider` | 推理强度滑块（chibi runner 形象） | 内置 |
