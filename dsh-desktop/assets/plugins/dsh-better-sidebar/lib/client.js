...
Mode: this is a continuable side conversation. Your answers stay in this side thread and are viewed in the side panel; they are never delivered into the parent session.`;
		...
		*/
		function apply(ctx) {
			...
			try {
				const syncPush = () => {
					const p = document.querySelector("[class*=_4ve_2a_panel]");
					const exp = document.querySelector("[class*=_4ve_2a_explorerRail]");
					const open = !!(p && !String(p.className).includes("Hidden"));
					// 推挤量 = workbench 面板 + explorer rail + 导航条自身余量（24px），
					// 缺 explorer 会把导航条推到资源管理器左侧过远处。
					const w = open && p ? Math.round(p.getBoundingClientRect().width) + (exp ? Math.round(exp.getBoundingClientRect().width) - 10 : 0) : 0;
					document.documentElement.style.setProperty("--dsh-official-push", w + "px");
				};
				syncPush();
				new MutationObserver(syncPush).observe(document.body, { attributes: true, subtree: true, attributeFilter: ["class", "style"] });
				setInterval(syncPush, 400);
			} catch (e) { console.warn("[dsh-better-sidebar] 官方推挤同步失败:", e); }
			ctx.effect(() => {
