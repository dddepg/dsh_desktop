...
export function revalidateChunksOnReactivate(): Promise<void> {
  testLoaders.clear()
  // HMR re-activation resets the auto-retry state: dispose every live loop
  // (their pending timers are cancelled) so no retry round outlives the
  // re-activated plugin. Fresh loads re-arm loops on demand.
  for (const loop of retryLoops.values()) loop.dispose()
  retryLoops.clear()
  dropVisibilityPokeIfIdle()
