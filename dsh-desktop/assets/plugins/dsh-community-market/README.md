...
> **本仓库的移植说明（DSH Desktop / myYangyunfan/dsh_desktop）**：本目录由上游
> [anywhere-labs/deepseek-harness-desktop](https://github.com/anywhere-labs/deepseek-harness-desktop)
> 的 `dsh-community-market` 源码构建（MIT），配套桥接插件
> `../dsh-market-desktop-bridge/` 提供 desktopProfiles / desktopPnpm /
> desktopPlugins / desktopActions 四个 host 服务。**再生成步骤**：
> 1. 克隆上游（GitHub 直连不通时走 `https://ghfast.top/https://github.com/...` 镜像）；
> 2. 在 `dsh-community-market/` 内用 esbuild 构建（宿主：`src/index.ts` → ESM
>    bundle `lib/index.js`，external `@deepseek-ai/*`+ajv/semver/yaml，alias
>    `sharp` → 惰性解析 shim；契约 schema 四个 JSON 以 esbuild JSON 导入内联，
>    上游运行期相对读 `docs/schemas/` 在 bundle 布局下层级错位；客户端：
>    `src/client/index.ts` → CJS bundle `lib/client.js`，按上游 tsdown 配置包
>    `window.__ModuleLoader__.load({ id, factory })` banner/footer）；
> 3. 产物拷入本目录 `lib/`，并执行
>    `node scripts/patch-community-market-restart.js` 打桌面监管重启补丁
>    （`[desktop-restart-fix]`，锚点变化时需重写该脚本）；
> 4. `docs/`（契约 schema 与适配器文档）随包分发；`package.json` 为本仓库
>    适配版（private + DSH Desktop 描述标记 + dsh.bundle/dsh.client 声明）。
