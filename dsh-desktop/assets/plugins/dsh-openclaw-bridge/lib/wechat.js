...
// 版本号随 package.json 走（修复⑨：不再硬编码 0.7.0，避免与发布版本漂移）。
...
const BOT_AGENT = "dsh-openclaw-bridge/" + PLUGIN_VERSION;
...
export function createWechatClient(options = {}) {
  ...
  function saveSession() {
    ...
    try {
      ...
      // mode 0o600：POSIX 下限制仅当前用户可读写（Windows 忽略）。
      // token 是明文敏感凭据，落盘权限尽量收紧（修复⑨）。
      if (session) writeFileSync(sessionFile, JSON.stringify(session), { mode: 0o600 });
      else if (existsSync(sessionFile)) writeFileSync(sessionFile, "{}", { mode: 0o600 });
    } catch {
      // 持久化失败不影响运行
    }
  }
  ...
  let loginLock = null; // 修复⑨：登录流程重入锁（快速连点「连接」只启动一轮 QR 流程）
  /** 开始扫码登录流程。 */
  async function startLogin() {
    if (loginLock) return; // 已有 QR 流程在跑，忽略重入
    loginLock = Promise.resolve();
    stopLoop();
    pendingVerify = "";
    setState("waiting-qr");
    try {
      ...
      loginLock = null; // 失败后允许重试
      throw err;
    }
    setState("waiting-scan");
    loginLock = pollQrStatus()
      .then(() => {}, () => {})
      .finally(() => {
        loginLock = null;
      });
    // 不 await：login 路由立即返回 waiting-scan，QR 轮询在后台继续
  }
  ...
  function logout() {
    loginLock = null; // 中止在途登录（轮询循环会随 state 变化退出）
    stopLoop();
    session = null;
    pendingVerify = "";
    qrcode = "";
