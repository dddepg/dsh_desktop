 ...
 */
export function defuseTemplateGroups(text) {
    return text
        .replace(/\{(?=\{)/g, "{\u200d")
        .replace(/\}(?=\})/g, "}\u200d");
}
...
export function buildSystemPromptAddition(params) {
    ...
    sections.push("", "## Recalled context for this query", "", "This is a context engine. The following was retrieved by semantic search for the current message:", "", "- **`<episodic_context>`** — Trimmed conversation traces from sessions that produced the knowledge nodes, ordered by time.", "- **`<knowledge_graph>`** — Relevant triples (TASK/SKILL/EVENT) and edges, grouped by community.", "- **Recent 5 turns** — Last turn in full, previous 4 turns as user+assistant text only.", "", "Read this context first. Use `gm_search` only if insufficient. Use `gm_record` to save new knowledge.");
    if (isRich) {
...
export function assembleContext(db, params) {
    ...
    const seen = new Set();
    const edges = allEdges.filter(e => selectedIds.has(e.fromId) && selectedIds.has(e.toId) && !seen.has(e.id) && seen.add(e.id));
    // 按社区分组节点
    const byCommunity = new Map();
    const noCommunity = [];
    for (const n of selected) {
        if (n.communityId) {
            if (!byCommunity.has(n.communityId))
                byCommunity.set(n.communityId, []);
    ...
    for (const node of topNodes) {
        ...
        if (!msgs.length)
            continue;
        const lines = msgs.map(m => `    [${m.role.toUpperCase()}] ${escapeXml(m.text.slice(0, 200))}`).join("\n");
