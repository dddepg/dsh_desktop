...
- [ ] WSL 内 `cat ~/.dsh-desktop/profiles/web/cordis.patch.yml` → 22 条 `- insert:`（12 个插件 + 10 个 `ui-skin-*` 皮肤行，皮肤默认 disabled）；
- [ ] UI 中：对话底部余额小部件、详情面板「文件」标签页（diff 查看）、「终端」标签页（复用 dsh-better-sidebar 的 node-pty 真终端）、侧边栏 GitGraph（外层/内层仓库可切换）、会话浮窗、插件市场、设置页「皮肤」10 款可预览切换、设置页「自定义提示词」可用；
...
- [ ] 重启后你的 dsh 设置页出现「WSL 后端」等全部配套栏目；补丁日志提示「已应用/无需变更」幂等；预设日志提示已同步 3 个内置 Agent 预设（未自动找到 dsh 包时用 `--dsh-package <包目录>` 指定）。
