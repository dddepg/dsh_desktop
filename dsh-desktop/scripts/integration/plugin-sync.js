...
function createPluginSync(ctx) {
  ...
  function sync() {
    ...
    try {
      ...
      // 内置皮肤：同步皮肤包文件并返回 ui-skin-* patch 行（默认禁用），
      // 注册与落盘在下方补丁注册步骤统一处理。
      const skinRows = syncSkinPackages({
        skinsRoot: path.join(appDir, 'assets', 'skins'),
        profileDir,
        log: (m) => log(m),
        fail: (m) => log(m),
      });
      ...
      const skinRegistration = registerSkinPatchEntries(registration.patch, skinRows, {
        onEntry: (m) => log(m),
      });
      if (registration.changed || skinRegistration.changed) {
        writeFileAtomic(patchFile, skinRegistration.patch);
