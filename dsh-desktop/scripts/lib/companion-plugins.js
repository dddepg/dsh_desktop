...
const COMPANION_PLUGINS = [
  ...
  // —— personal/ezio 专属配套插件 ——
  // 侧边栏 GitGraph（本仓库内置）：VSCode 风格分支地图概览，自动识别
  // 外层 + 内层双仓库并支持一键切换；非 bundle，经 patch 行注册。
  { id: 'gitgraph', name: '@deepseek-ai/dsh-gitgraph' },
  // 皮肤切换（本仓库内置）：设置页「皮肤」tab 列出并切换内置皮肤，
  // host 半边以 Typert Remote 重写 cordis.patch.yml 的 ui-skin-* 激活行；
  // 非 bundle，经 patch 行注册。
  { id: 'skin-switch', name: '@deepseek-ai/dsh-skin-switch' },
  // 推理强度滑块（本仓库内置）：Claude 风格滑杆弹层 + HanaAyane chibi
  // runner 拇指形象；bundle 插件，经 profile manifest 装配。
