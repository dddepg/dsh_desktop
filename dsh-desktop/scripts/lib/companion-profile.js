...
// ---------------------------------------------------------------------------
// 内置皮肤（assets/skins/<id>/）：皮肤包是带 skin.json 元数据的客户端插件，
// 按 package.json 的 name（含 scope）落到 profile node_modules，并以
// skin.json wiring.id 写 ui-skin-* patch 行（默认 disabled，用户在设置 →
// 皮肤 一键启用；dsh-skin-switch 重写激活行）。
// ---------------------------------------------------------------------------

// 同步进 profile 的皮肤固定文件清单（含许可/说明；存在才拷）。
...
// 皮肤包自带的子目录（lib 入口、preview 预览图等），整树复制。
const SKIN_SUBDIRS = ['lib', 'preview', 'src', 'assets', 'vendor', 'node_modules', 'data'];

// 皮肤 patch 行的 loader id 必须匹配 ui-skin-<name> 形态。
const SKIN_ID_RE = /^ui-skin-[\w-]+$/;
...
/**
 * 把 assets/skins 下的内置皮肤包同步进 profile web node_modules，并返回
 * ui-skin-* patch 行（默认 disabled）。皮肤包身份校验：package.json name
 * 必须含 scope（与第三方皮肤同布局），skin.json wiring.id 必须匹配
 * SKIN_ID_RE。已存在同名 skin 目录不比对版本（皮肤随客户端整体发布）。
 * @param {Object} opts
 * @param {string} opts.skinsRoot  assets/skins 目录
 ...
 * @returns {Array<{id:string,name:string,disabled:true}>}
 */
function syncSkinPackages(opts) {
  const { skinsRoot, profileDir, log, fail, plan, dryRun = false } = opts;
  const rows = [];
  if (!fs.existsSync(skinsRoot)) return rows;
  const profileModules = path.join(profileDir, 'node_modules', '@deepseek-ai');
  for (const entry of fs.readdirSync(skinsRoot, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    const src = path.join(skinsRoot, entry.name);
    let pkg = null;
    try { pkg = JSON.parse(fs.readFileSync(path.join(src, 'package.json'), 'utf8')); } catch { pkg = null; }
    if (!pkg || typeof pkg.name !== 'string' || !pkg.name.includes('/')) continue;
    let skin = null;
    try { skin = JSON.parse(fs.readFileSync(path.join(src, 'skin.json'), 'utf8')); } catch { skin = null; }
    const rowId = skin && skin.wiring && typeof skin.wiring.id === 'string' ? skin.wiring.id : '';
    if (!SKIN_ID_RE.test(rowId)) continue;
    const dest = path.join(profileModules, '..', pkg.name);
    if (dryRun) {
      if (plan) plan(`dry-run: 将安装皮肤 ${pkg.name} → ${dest}（${rowId}，默认禁用）`);
      rows.push({ id: rowId, name: pkg.name, disabled: true });
      continue;
    }
    ...
    for (const f of SKIN_PACKAGE_FILES) {
      ...
      try {
        ...
        if (fail) fail('同步皮肤文件失败 ' + sf + ': ' + err.message);
      }
    }
    for (const sub of SKIN_SUBDIRS) {
      ...
      try {
        ...
        if (fail) fail('同步皮肤目录失败 ' + sdir + ': ' + err.message);
      }
    }
    rows.push({ id: rowId, name: pkg.name, disabled: true });
    if (log) log('已安装皮肤 ' + pkg.name + '（' + rowId + '，默认禁用）');
  }
  return rows;
}

/**
 * 把内置皮肤注册进 profile patch 层（幂等，纯文本函数）：每个皮肤写一条
 * `- insert:` 内层条目并带 disabled: true（用户需在 设置 → 皮肤 启用）。
 * id 已存在（含用户手写条目/禁用块）则跳过，尊重用户配置。
 * @param {string} patch cordis.patch.yml 原文
 * @param {Array<{id:string,name:string}>} rows
 * @param {{onEntry?:(msg:string)=>void}} [hooks]
 * @returns {{ patch: string, changed: boolean, added: string[] }}
 */
function registerSkinPatchEntries(patch, rows, hooks = {}) {
  const { onEntry } = hooks;
  let text = String(patch || '');
  let changed = false;
  const added = [];
  for (const row of rows) {
    const idRe = new RegExp('(?:^|\\n)\\s*-?\\s*id\\s*:\\s*' + escRegExp(row.id) + '(?![A-Za-z0-9_.-])');
    if (idRe.test('\n' + text)) continue; // 已存在：跳过（尊重用户已有条目）
    const block = `- insert:\n    - id: ${row.id}\n      name: '${row.name}'\n      disabled: true\n`;
    if (/^\s*\[\]\s*$/m.test(text)) text = text.replace(/\[\]/m, block);
    else if (text.trim() === '') text = PATCH_HEADER + block;
    else text = text.replace(/\s*$/, '\n') + block;
    changed = true;
    added.push(row.id);
    if (onEntry) onEntry('已添加皮肤条目 ' + row.id + ' → ' + row.name + '（默认禁用）');
  }
  return { patch: text, changed, added };
}
