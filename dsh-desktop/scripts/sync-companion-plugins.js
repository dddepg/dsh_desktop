...
const {
  ...
  syncSkinPackages, registerSkinPatchEntries,
...
function syncPlugins(home, dryRun, dshPkgDir) {
  ...
  const skinRows = syncSkinPackages({
    skinsRoot: path.join(__dirname, '..', 'assets', 'skins'),
    profileDir,
    dryRun,
    log: (m) => log(m),
    fail: (m) => warn(m),
  ...
  // 内置皮肤注册（幂等，默认禁用；在 registerCompanionPatchEntries 之后
  // 追加，与插件条目共用最后统一原子写一次）。
  const skinReg = registerSkinPatchEntries(patch, skinRows, {
    onEntry: (m) => log(m),
  });
  patch = skinReg.patch;
  if (skinReg.changed) changed = true;
